...
| Cloud | AWS EC2 (`t3.medium`, Ubuntu 22.04) | Free tier éligible, écosystème Terraform mature |
...
### 3.4 Provisioning Terraform (VM + VPC + SG)

- VPC 10.0.0.0/16, 2 subnets publics multi-AZ.
- EC2 `t3.medium` avec `user_data` : Docker + K3s + kubectl + Helm installés automatiquement.
- Security Group : 22 (SSH), 80 (HTTP), 443, 6443 (K3s API), 30000-32767 (NodePort).
- Elastic IP associée pour conserver l'adresse publique en cas de redimensionnement.
...
Comparaison tailles :
- Single-stage (FROM node:20) : ~1,11 Go
- Multi-stage (alpine, prod deps) : 153 Mo
- Gain : **~87 %**.
...
Bonus réalisés : **3/4** (multi-stage Docker, Terraform, monitoring Prometheus/Grafana).
