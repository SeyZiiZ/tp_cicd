...
- ![Image GHCR](screenshots/04_ghcr_packages.png)
...
*(À compléter au fil du TP — exemples de sections à remplir)*

### 5.1 _Exemple_ : timeout SSH depuis GitHub Actions
- **Symptôme** : `dial tcp: i/o timeout`
- **Cause** : Security Group restreint à IP admin, IP GitHub runners variable.
- **Correction** : règle SSH ouverte sur `0.0.0.0/0` pour le runner CI OU utilisation de `appleboy/ssh-action` avec `port 22` par défaut + vérification `EC2_HOST` à jour après `terraform apply`.

### 5.2 _Exemple_ : OOM MongoDB sur `t3.small`
- **Symptôme** : pods `mongo` OOMKilled après 20 min.
- **Cause** : K3s + Mongo + Prometheus sur 2 Go RAM.
- **Correction** : passage à `t3.medium` dans `variables.tf` OU désactivation temporaire des node-exporters inutiles.

### 5.3 _Exemple_ : Image pull `ghcr.io/... forbidden`
- **Cause** : le package est privé par défaut → la VM n'a pas de credentials.
- **Correction** : création d'un `docker-registry` secret (`ghcr-pull`) attaché au Deployment via `imagePullSecrets`, alimenté par un PAT avec scope `read:packages` (`GHCR_PULL_TOKEN`).
